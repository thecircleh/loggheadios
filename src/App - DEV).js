...
const saveMatchData = useCallback(async (showAlert = false) => {
  ...
  try {
    ...
    const matchPayload = {
      ...
      currentSet: isMatchOver ? null : (matchSettings?.currentSet || 1),
...
const syncMatchState = useCallback(async () => {
  ...
  // TODO: sync-state endpoint not yet implemented
  // State is currently persisted via auto-save to coach-match-analytics
  console.log("ℹ️  State auto-saved via coach-match-analytics PUT endpoint");
  /*
  try {
...
const processSetEnding = useCallback(async (winner, winnerName, newOurSets, newOpponentSets, isMatchOver) => {
  // 1. ATOMIC LOCK
  // If we are already processing, bail immediately.
  if (setEndingInProgressRef.current) {
    console.warn("⚠️ [ProcessSetEnding] Blocked: Operation already in progress.");
    return;
  }

  try {
    setEndingInProgressRef.current = true;
    setIsSetEndingInProgress(true);

    // Snapshot variables to ensure they don't change during async awaits
    const finalSetNumber = matchSettings.currentSet;
    const finalOurScore = ourScore;
    const finalOpponentScore = opponentScore;

    console.log(`🏆 [Coach Match] Finalizing Set ${finalSetNumber}. Match Over: ${isMatchOver}`);

    // 2. STAGE 1: FORCE ANALYTICS SYNC
    // This ensures rotationPlusMinus and playerPlusMinus are written to 'currentSet' in DB
    if (!collaborativeMode || isMatchOwner()) {
      console.log("💾 Step 1/3: Force syncing analytics...");
      await syncMatchState(); 
      // Mandatory wait for DB consistency
      await new Promise(resolve => setTimeout(resolve, 400));
    }

    // 3. STAGE 2: ARCHIVE SET (The critical backend move)
    if ((!collaborativeMode || isMatchOwner()) && matchSettings?.mode?.toLowerCase() !== 'coach') {   
      console.log(`📦 Step 2/3: Archiving Set ${finalSetNumber}...`);
      await axios.post(`${API_URL}/api/coach-match-analytics/${currentMatchId}/end-set`, {
        finalOurScore,
        finalTheirScore: finalOpponentScore,
    ...
    // 4. STAGE 3: UPDATE MAIN MATCH RECORD
    const currentSetScore = {
      setNumber: finalSetNumber,
      ourScore: finalOurScore,
      opponentScore: finalOpponentScore,
    };
    const updatedSetScores = [...setScores, currentSetScore];

    if (isMatchOver) {
      console.log("🏁 Step 3/3: Match Over. Finalizing document.");
      if (!collaborativeMode || isMatchOwner()) {
        ...
        // Remove active match from user profile
        await axios.put(`${API_URL}/api/users/${user.id}/match-id`, { currentMatchId: null }, {
          headers: { Authorization: `Bearer ${token}` },
          withCredentials: true,
        });
      }
      ...
      const nextSetNum = finalSetNumber + 1;
      console.log(`🔄 Step 3/3: Preparing Set ${nextSetNum}`);
      ...
      if (!collaborativeMode || isMatchOwner()) {
        await axios.put(`${API_URL}/api/matches/${currentMatchId}`, {
          ...
          setScores: updatedSetScores,
          isSetLive: true,
          teamStats: { ourEarned: 0, ourError: 0, oppEarned: 0, oppError: 0 }
        }, { headers: { Authorization: `Bearer ${token}` }, withCredentials: true });
      }
      ...
      // Update matchSettings to trigger court reset
      setMatchSettings(prev => ({ ...prev, currentSet: nextSetNum }));
    ...
    console.error("❌ Critical Error in processSetEnding:", err);
  } finally {
    // 6. LONG COOL-DOWN
    // We stay locked for 2.5 seconds to let the backend finish all state transitions
    setTimeout(() => {
      setEndingInProgressRef.current = false;
      setIsSetEndingInProgress(false);
      console.log("🔓 Set ending lock released.");
    }, 2500);
  }
}, [
  matchSettings, ourScore, opponentScore, setScores, currentMatchId, token, user,
  navigate, collaborativeMode, isMatchOwner, syncMatchState, clearCourt, resetGamesPlayedForNewSet
]);
...
/**
 * 🏆 END-MATCH HANDLER (Coach Mode Only)
 * Aggregates all set statistics and finalizes the match in the analytics database.
 * Called when match is completely over (all sets finished).
 */
...
const handleSetEndingAsOwner = useCallback(async (winner, winnerName, newOurSets, newOpponentSets, isMatchOver) => {
 ...
 if (matchSettings?.mode === "Coach") {
    console.log("🎯 COACH MODE DETECTED: Bypassing individual handler, using CoachCourt modal");
    setEndingInProgressRef.current = false;
    return;
  } 
...
const startCoachMatch = useCallback(
  async (teamName, overrides = {}) => {
    isCreatingNewMatchRef.current = true;
    
    // Start fresh - don't merge with stale matchSettings from previous match!
    const newSettings = {
      teamName,
      opponentName: overrides?.opponentName || "Opponent",
      totalSets: Math.max(1, overrides?.totalSets),
      pointsNonDeciding: overrides?.pointsNonDeciding,  
      pointsDeciding: overrides?.pointsDeciding,
      coachMode: true,
      mode: 'Coach',
      collaborativeMode: { enabled: false },
      playAllSets: !!overrides?.playAllSets,
      ...overrides,  // Apply any other overrides
    };
...
useEffect(() => {
  ...
  const rawNonDeciding = matchSettings.pointsNonDeciding;
  const rawDeciding = matchSettings.pointsDeciding;
  ...
  const pointsNonDeciding = toPosInt(rawNonDeciding);
  const pointsDeciding = toPosInt(rawDeciding);

  // No hardcoded fallback: if settings are missing, refuse to auto-end
  if (!pointsNonDeciding || !pointsDeciding) {
    console.warn("⚠️ Auto set-end disabled: missing points settings", {
      pointsNonDeciding: rawNonDeciding,
      pointsDeciding: rawDeciding,
      matchId: currentMatchId,
    });
    return;
  }

  const currentSet = Number(matchSettings.currentSet || 1);
  const totalSets = Number(matchSettings.totalSets || 3);
  const playAllSets = !!matchSettings.playAllSets;

  const isDecidingSet = currentSet === totalSets;

  const requiredPoints = playAllSets
    ? pointsNonDeciding
    : (isDecidingSet ? pointsDeciding : pointsNonDeciding);

  const maxScore = Math.max(ourScore, opponentScore);
  const scoreDiff = Math.abs(ourScore - opponentScore);

  if (maxScore >= requiredPoints && scoreDiff >= 2) {
    ...
    // Use the same path you already use (prompts/locks remain intact)
    if (collaborativeMode && !isMatchOwner()) {
      requestSetEndingLock?.();
      return;
    }
    ...
    const handleBeforeUnload = async () => {
      ...
      try {
        ...
        try {
          ...
          // TODO: sync-state endpoint not yet implemented - state persists via auto-save
          // navigator.sendBeacon(`${API_URL}/api/matches/${currentMatchId}/sync-state`, blob);
          console.log("ℹ️  State persisted via auto-save on page unload");
        } catch (beaconErr) {
          console.error("beacon save setup failed:", beaconErr);
        }
      }
    };
...
useEffect(() => {
  ...
  console.log(`Match ${currentMatchId} is finalized`);

  // ✅ Don't wipe state if we're trying to view post-match pages
  const path = location?.pathname || "";
  const isPostMatchView =
    path.startsWith("/match-summary") ||
    path.startsWith("/coach-stats") ||
    path.startsWith("/coachstats");

  if (isPostMatchView) {
    console.log("Post-match view route detected - preserving state");
    setLoadingMatch(false);
    setIsRestoringMatch(false);
    isRestoringMatchRef.current = false;
    return;
  ...
  console.log("Clearing currentMatchId from user profile");
  try {
    await axios.put(
      `${API_URL}/api/users/${user.id}/match-id`,
      { currentMatchId: null },
 ...
 {location.pathname !== "/profile" && location.pathname !== "/login" && location.pathname !== "/register" && (
  ...
  >
    {showHeader ? "Hide Header" : "Show"}
          ...
          {isPortrait && !isDismissed && location.pathname === "/classic" && (
            <div style={{
              position: "fixed",
              top: 0,
              left: 0,
...
<Routes>
  ...
  <Route
    ...
    element={
      <PrivateRoute>
        {/* keep your existing "/" element logic here (loadingMatch, restore UI, VolleyballCourt, etc.) */}
        {/* IMPORTANT: paste your current "/" element body here unchanged */}
        { /* ... */ }
      </PrivateRoute>
    }
  />
