...
}, [courtPlayers, positionMapping, updateCourtPositions, setBenchPlayers, addActionLogEntry, token, collaborativeMode, isConnected, syncCourtPlayers]);
...
, []);
  ...
  useEffect(() => {
    const fetchRoster = async () => {
      ...
      try {
        ...
        // Initialize on/off status: players in courtPlayers are ON, rest are OFF
        const newStatus = {};
        roster.forEach(player => {
          const isInCourt = courtPlayers.some(cp => cp?._id === player._id);
          newStatus[player._id] = isInCourt;
        });
  ...
  const handlePlayerToggle = useCallback((playerId) => {
    setPlayerOnOffStatus(prev => {
      // Count how many players are currently ON
      const currentOnCount = Object.values(prev).filter(Boolean).length;
      
      // If trying to turn ON and already have 6 ON players, prevent it
      if (!prev[playerId] && currentOnCount >= 6) {
        showActionToast('Maximum 6 players on court. Turn OFF a player first.', 'warning');
        return prev; // Don't change status
      }

      const newStatus = { ...prev };
      newStatus[playerId] = !newStatus[playerId];

      const rosterPlayer = allRosterPlayers.find(p => p._id === playerId);
      if (!rosterPlayer) return prev;
      ...
      if (newStatus[playerId]) {
        ...
        setCourtPlayers(prevPlayers => {
          ...
          return [...prevPlayers, rosterPlayer];
        });
      } else {
        // Remove from court
        setCourtPlayers(prevPlayers =>
          prevPlayers.filter(p => p._id !== playerId)
        );
      }

      return newStatus;
    });
  }, [allRosterPlayers, setCourtPlayers, showActionToast]);
...
case 'KILL':
  // Start kill mode - show inline menu to select assist players
  setKillMode('kill');
  setKillInitiatorIndex(playerIndex);
  setSelectedAssistPlayers(new Set()); // Start with no assists selected
  setSelectedActionFamily(null);
  return; // Exit early, don't process normal kill logic yet
...
const getAvailableActions = (playerIndex) => {
  ...
  if (blockMode && blockInitiatorIndex === playerIndex) {
    // Get all other ON players who aren't liberos (no position tracking in this mode)
    const otherPlayers = courtPlayers
      ...
      .filter(({ player, index }) => {
        return (
          ...
          player.name !== "?" && 
          !player.isLibero &&
          index !== playerIndex // Don't include the initiator themselves
        );
      });
    ...
    return [
      {
        category: `Block ${blockMode === 'assist' ? 'Assist' : 'Error'} - Select Players:`,
        actions: [
      ...
      return [
        {
          ...
          actions: [
            ...
            { key: "BACK_TO_ACTIONS", label: "CANCEL", variant: "secondary" },
          ],
        },
      ];
...
const splitNameForDisplay = (name, maxTotalLength = 18, maxLineLength = 14) => {
  ...
  const actionButtonStyle = (variant = 'primary') => ({
    ...
    minWidth: '40px',
    minHeight: '40px',
    backgroundColor: variant === 'danger' ? '#dc3545' : variant === 'success' ? '#28a745' : variant === 'warn' ? 'orange' : '#007AFF',
    color: '#ffffff',
    whiteSpace: 'nowrap',
    transform: 'scale(1)',
        ...
        {allRosterPlayers.length > 0 ? (
          ...
          [...allRosterPlayers].sort((a, b) => {
            ...
            if (isOn && playerIndex >= 0) {
              ...
              if (playerIndex === selectedPlayerForActions) {
                // This player has submenu open - show full actions (including submenu)
                  ...
                  >
                    {actionGroups.map((group, groupIndex) => (
                      <div key={group.category} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        {/* Show category label for block selections */}
                        {group.category.includes('Block') && (
                          <div style={{
                            fontSize: '11px',
                            fontWeight: '700',
                            color: '#555',
                        ...
                        <div style={{
                          ...
                          {group.actions.map(action => (
                            <button
                              ...
                              style={getActionButtonStyleWithFeedback(action.variant, `player-${playerIndex}-${action.key}`)}
                              title={action.label}
                            >
                              {action.label}
                            </button>
