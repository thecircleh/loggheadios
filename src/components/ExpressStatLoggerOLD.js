...
const assistWaitingServeSideRef = useRef(null); // Track recent actions for toast display
...
const triggerNonBlockingSave = useCallback((reason = 'action') => {
  ...
  saveTimeoutRef.current = setTimeout(async () => {
    ...
    // Respect minimum interval between saves (2 seconds)
    if (timeSinceLastSave < 2000) {
      const waitMs = 2000 - timeSinceLastSave;
...
const addActionLogEntry = useCallback((action, options = {}) => {
  ...
  // 🔥 ONLY update local UI state - DON'T trigger saves
  ...
  // Don't call triggerNonBlockingSave here!
  // Action log updates alone won't trigger saves anymore
}, [collaborativeMode, setActionLog, user]);

const setCurrentServeSideWithSync = useCallback((newServeSide) => {
  if (collaborativeMode && isConnected) {
    // Set local override first
    setLocalStateOverride({
      currentServeSide: newServeSide,
      timestamp: Date.now()
    });
    
    // Clear the override after a short delay
    if (localStateTimeoutRef.current) {
      clearTimeout(localStateTimeoutRef.current);
    }
    localStateTimeoutRef.current = setTimeout(() => {
      setLocalStateOverride(null);
    }, 1000);
    
    // COLLABORATIVE MODE: Send to server, but also update local immediately
    setCurrentServeSide(newServeSide);
    setBallSide(newServeSide);
    
    const syncSuccess = updateCollaborativeServeSide(newServeSide);
    
    if (!syncSuccess) {
      console.warn('Failed to sync serve side change');
      showActionToast('Failed to sync serve change', 'error');
    }
  } else {
    // NON-COLLABORATIVE MODE: Update directly
    setCurrentServeSide(newServeSide);
    setBallSide(newServeSide);
  }
}, [
...
const setBallStateWithSync = useCallback((newBallState) => {
  console.log('🎯 setBallStateWithSync called:', {
    newBallState,
    collaborativeMode,
    isConnected
  });
  ...
  // Set a brief local override to prevent incoming collaborative changes from overriding our immediate change
  setLocalStateOverride({
  ...
  if (collaborativeMode && isConnected) {
    console.log('📤 Sending ball state update to other users');
    const syncSuccess = updateCollaborativeBallState(newBallState, {
      ballSide: ballSide,
      currentServeSide: currentServeSide
    });
    
    if (!syncSuccess) {
      console.warn('Failed to send collaborative ball state update to other users');
      showActionToast('Failed to sync state change', 'error');
    } else {
      console.log('✅ Ball state update sent successfully');
    }
  }
}, [
  setBallState,
...
const onOurPointRef = useRef(onOurPoint);
const onOpponentPointRef = useRef(onOpponentPoint);
const setTeamStatsRef = useRef(setTeamStats);
const setBallStateRef = useRef(setBallState);
const setCurrentServeSideRef = useRef(setCurrentServeSide);
const setBallSideRef = useRef(setBallSide);
...
useEffect(() => {
  onOurPointRef.current = onOurPoint;
  onOpponentPointRef.current = onOpponentPoint;
  setTeamStatsRef.current = setTeamStats;
  setBallStateRef.current = setBallState;
  setCurrentServeSideRef.current = setCurrentServeSide;
  setBallSideRef.current = setBallSide;
...
useEffect(() => {
  ...
  registerStateCallbacks({
    // Handle ball state changes from other users
    onBallStateChange: (newBallState, data) => {
      console.log('🎯 ExpressStatLogger: Received ball state change:', {
        newBallState,
        fromUser: data.username,
        currentLocal: ballState
      });
      
      // Skip if this is our own change (already applied locally)
      if (data.userId === getCurrentUserId()) {
        console.log('⏩ Skipping own ball state change');
        return;
      }
      
      // Check if we have a local override (user just made a change)
      if (localStateOverride?.ballState && 
          (Date.now() - localStateOverride.timestamp) < 500) {
        console.log('⏩ Skipping due to local override');
        return;
      }
      
      // Apply the remote ball state change
      console.log('✅ Applying remote ball state:', newBallState);
      setBallState(newBallState);
      
      // Also update ball side if provided
    ...
    // Handle serve side changes from other users
    onServeSideChange: (newServeSide, data) => {
      console.log('🎯 ExpressStatLogger: Received serve side change:', {
        newServeSide,
        fromUser: data.username,
        currentLocal: currentServeSide
      });
      
      // Skip if this is our own change
      if (data.userId === getCurrentUserId()) {
        console.log('⏩ Skipping own serve side change');
        return;
      }
      
      // Check if we have a local override
      if (localStateOverride?.currentServeSide && 
          (Date.now() - localStateOverride.timestamp) < 500) {
        console.log('⏩ Skipping due to local override');
        return;
      }
      
      // Apply the remote serve side change
      console.log('✅ Applying remote serve side:', newServeSide);
      setCurrentServeSide(newServeSide);
      setBallSide(newServeSide);
    },
...
useEffect(() => {
  ...
  const handleAssistWaitingStarted = (data) => {
    ...
    if (data.assistPlayer) {
      ...
      assistWaitingServeSideRef.current = data.serveSideAtRallyStart;
  ...
  const handleAssistKillCompleted = (data) => {
    ...
    assistWaitingServeSideRef.current = null;
...
useEffect(() => {
  ...
  const handleServeSideUpdated = (data) => {
    const currentUserId = getCurrentUserId();
    
    // Skip if this is our own update
    if (data.updatedBy?.userId === currentUserId) {
      console.log('📤 Skipping own serve side update');
      return;
    }
    
    console.log('📥 Receiving serve side update from another user:', {
      newServeSide: data.serveSide,
      updatedBy: data.updatedBy?.username
    });
    
    // Apply the serve side update
    setCurrentServeSide(data.serveSide);
    setBallSide(data.serveSide);
    
    showActionToast(
      `${data.updatedBy?.username || 'Another user'} changed serve to ${data.serveSide === 'our' ? 'Our team' : 'Opponent'}`, 
      'info'
    );
    
    console.log('✅ Applied serve side update from remote user');
  };
  
  socket.on('serve_side_updated', handleServeSideUpdated);
  
  return () => {
    socket.off('serve_side_updated', handleServeSideUpdated);
  };
}, [
  collaborativeMode, 
  socket, 
...
useEffect(() => {
  ...
  const handleBallStateUpdated = (data) => {
    const currentUserId = getCurrentUserId();
    
    // Skip if this is our own update
    if (data.updatedBy?.userId === currentUserId) {
      console.log('📤 Skipping own ball state update');
      return;
    }
    
    console.log('📥 Receiving ball state update from another user:', {
      newBallState: data.ballState,
      ballSide: data.context?.ballSide,
      currentServeSide: data.context?.currentServeSide,
      updatedBy: data.updatedBy?.username
    });
    
    // Apply the ball state update
    setBallState(data.ballState);
    
    // Also update ball side and serve side if provided
    if (data.context?.ballSide !== undefined) {
      setBallSide(data.context.ballSide);
    }
    
    if (data.context?.currentServeSide !== undefined) {
      setCurrentServeSide(data.context.currentServeSide);
    }
    
    // Show notification for state changes
    const stateLabel = data.ballState === 'serve' ? 'Serve' : 
                       data.ballState === 'inplay' ? 'Rally' : data.ballState;
    showActionToast(
      `${data.updatedBy?.username || 'Another user'} changed state to ${stateLabel}`, 
      'info'
    );
    
    console.log('✅ Applied ball state update from remote user');
  };
  
  socket.on('ball_state_updated', handleBallStateUpdated);
  
  return () => {
    socket.off('ball_state_updated', handleBallStateUpdated);
  };
}, [
  collaborativeMode, 
  socket, 
...
useEffect(() => {
  ...
  const handleBlockSelectionStarted = (data) => {
    ...
    const canParticipate = courtPlayers
      .filter((p, idx) => {
        const pos = getVolleyballPosition(idx);
        return ['2', '3', '4'].includes(pos) && p?._id;
      })
...
const logStat = useCallback(async (player, label, statKeys, actionText, options = {}) => {
  ...
  // ============================================
  const actionEntry = actionText || formatPlayerAction(player, label);
  addActionLogEntry(actionEntry, {
    type: 'stat_log',
    meta: { playerId: player._id, label, statKeys }
  });

  // ============================================
  // Try collaborative if enabled
  // ============================================
  ...
  // ============================================
  // Fallback: Local logging (ALWAYS succeeds)
  // ============================================
  ...
  try {
    await logAndSyncStat({
      ...
      label,
      statKeys,
      setActionLog: addActionLogEntry,
      setPlayerStats,
      playerStats,
      currentMatchId,
...
const creditGamesPlayed = useCallback(async (rawPlayerId, context = 'express', { force = false } = {}) => {
 ...
 const toggleCollaborativeMode = useCallback(async (enabled) => {
  ...
  // Update the match in the database FIRST
  if (currentMatchId) {
    try {
      const response = await axios.put(`${API_URL}/api/matches/${currentMatchId}`, {
        collaborativeMode: {
          enabled: enabled,
          allowedUsers: [],
          maxUsers: 15
      ...
      // ✅ UPDATE: Now update the local matchSettings state so App.js knows about the change
      if (setMatchSettings && typeof setMatchSettings === 'function') {
        ...
        console.log(`Local matchSettings updated: collaborative mode ${enabled ? 'enabled' : 'disabled'}`);
      } else {
        console.warn('setMatchSettings not available - local state may be out of sync');
      }
      
      // If enabling, try to join immediately after state update
     if (enabled) {
  // The useEffect above will handle joining automatically
  ...
  leaveMatch();
    ...
    console.error('No currentMatchId available for collaborative mode update');
...
const handleExpressRotation = useCallback(async () => {
  ...
  if (liberoToSub && liberoToSub.replacedPlayer) {
    ...
    try {
      ...
      // 🔥 NEW: Sync the combined substitution + rotation to other users
      if (collaborativeMode && isConnected) {
        syncCourtPlayers(rotatedPlayers, {
          action: 'libero_substitution_with_rotation',
          liberoOut: { id: libero._id, name: libero.name },
          playerIn: { id: replacedPlayer._id, name: replacedPlayer.name },
          rotationType: 'clockwise',
          fromPosition: '5',
          toPosition: '4',
          timestamp: new Date().toISOString()
      ...
      // FIXED: Use collaborative action log
      addActionLogEntry(
        `Express: Auto-substitution + rotation - ${replacedPlayer.name} in for ${libero.name}, then rotated to position 4`,
        {
...
}, [courtPlayers, positionMapping, updateCourtPositions, setBenchPlayers, addActionLogEntry, token, rotateExpressPositions, collaborativeMode, isConnected, syncCourtPlayers]);
...
const handleStartBlockSelection = useCallback(async (playerIndex, blockType) => {
  ...
  if (!['2', '3', '4'].includes(pos)) {
    showActionToast('Only front row players can participate in blocks', 'error');
    return;
  }
  

  
  // Set local block mode
...
const handleBlockPlayerToggle = useCallback((playerIndex) => {
  ...
  // ✅ NO ASSIGNMENT CHECKS - Allow toggling any front row player for blocks
...
const submitBlock = useCallback(async () => {
  ...
  if (blockMode === 'assist') {
    ...
    onOpponentPoint && onOpponentPoint(true);
  ...
  setBallStateWithSync("serve");
  const newServeSide = blockMode === 'assist' ? 'our' : 'opponent';
  setCurrentServeSideWithSync(newServeSide);
  
  if (blockMode === 'assist' && wasOpponentServing) {
    setTimeout(() => {
  ...
  setTeamStats,
  updateCollaborativeScore,
  setBallStateWithSync,
  setCurrentServeSideWithSync,
  handleExpressRotation,
  socket,
  scoreCooldownActive,
  scoreCooldownRemaining,
...
const handleAssistTimeout = useCallback(async () => {
  ...
  const effectiveServeSide = assistWaitingServeSideRef.current || currentServeSide;
  ...
  setBallStateWithSync("serve");
  setCurrentServeSideWithSync("our");
...
const ignoreKill = useCallback(async () => {
  ...
  await logStat(assistPlayer, "Assist", ["sets", "assists"], 
  ...
  // FIXED: Use collaborative-aware state setters
  setBallStateWithSync("serve");
  setCurrentServeSideWithSync("our");
...
, []);
  ...
  useEffect(() => {
    ...
    fetchRoster();
  }, [match?.teamName, token]);

  // Save slot order to localStorage whenever courtPlayers changes
  useEffect(() => {
 ...
 const handleRallyStart = useCallback(() => {
  ...
  // Move to rally with sync
  setBallStateWithSync('inplay');
  
  // Mid-rally, we don't track possession with ballSide; clear it
  setBallSide(null);
  
  // Log it
  addActionLogEntry('Manual: Rally started', {
    type: 'rally_start',
    meta: { type: 'rally_start', serveSideAtStart: currentServeSide }
  });
}, [currentServeSide, setBallStateWithSync, setBallSide, addActionLogEntry]); 

 // Handle player actions - updated to use courtPlayers

const handlePlayerAction = useCallback(async (playerIndex, actionType, passGrade = null) => {
  const player = courtPlayers[playerIndex];
  if (!player || !isPlayerActive(playerIndex)) return;
...
if (actionType === 'ASSIST') {
  // Show modal for both collaborative and non-collaborative modes
  setAssistMode(true);
  setSelectedAssistPlayer(playerIndex);
  
  // If collaborative, notify others that assist selection is in progress
  if (collaborativeMode && isConnected && socket) {
...
case 'SERVE':
  ...
  console.log('🎾 Will change ball state to: inplay');
  break;
    ...
    case 'SERVE_ERR':
     ...
     case 'REC':
        ...
        if (passGrade === 1) statKeys.push('receive_1');
    ...
    case 'REC_ERR':
      ...
      case 'DIG':
        label = "Dig";
        statKeys = ["digs"];
        // NEW: map passGrade → dig_1/2/3
        if (passGrade === 1) statKeys.push('dig_1');
...
case 'KILL':
  console.log('🎯 KILL button clicked!', {
    ...
    assistWaitingPlayer: assistWaitingPlayer?.name,
    flagSet: assistKillSelectedRef.current
  });
  
  if (collaborativeMode && collaborativeAssistWaiting && assistWaitingPlayer) {
    console.log('🔵 Processing collaborative kill (assist already logged)');
    
	 if (assistWaitingPlayer._id === player._id) {
      showActionToast(`${player.name} already has the assist - select a different player for the kill`, 'error');
      triggerActionFeedback(`player-${playerIndex}-KILL`, 'error');
      return; 
    }
    // ✅ Set flag FIRST
    assistKillSelectedRef.current = true;
    ...
    // Store current assist player and serve side
    const currentAssistPlayer = assistWaitingPlayer;
    const currentServeSide = assistWaitingServeSideRef.current;
    ...
    // ✅ LOG KILL STAT ONLY (assist already logged when button was clicked)
    console.log('📊 Logging kill stat for:', player.name);
    await logStat(player, "Kill", ["kills", "attacks", "points"],
      `${player.name} (#${player.number}) → Kill`);
    ...
    // Add action log
    addActionLogEntry(
      `Assist by ${currentAssistPlayer.name} (#${currentAssistPlayer.number}) → Kill by ${player.name} (#${player.number})`,
      {
        type: 'assist_kill_combo',
        meta: {
          type: 'assist_kill_combo',
          assistPlayer: { id: currentAssistPlayer._id, name: currentAssistPlayer.name, number: currentAssistPlayer.number },
          killPlayer: { id: player._id, name: player.name, number: player.number }
        }
      }
    );
    ...
    if (socket) {
      socket.emit('assist_kill_completed', {
        matchId: currentMatchId,
        assistPlayer: {
          _id: currentAssistPlayer._id,
          name: currentAssistPlayer.name,
          number: currentAssistPlayer.number
        },
        killPlayer: {
          _id: player._id,
          name: player.name,
    ...
    showActionToast('Kill recorded!', 'success');
    ...
    return; // ✅ EXIT - don't process normal kill logic
  }
  
  // Normal kill handling (when not in assist mode)
  label = "Kill";
  ...
  if (shouldScore === "our") {
    ...
    if (collaborativeMode && isConnected) {
      const success = updateCollaborativeScore('our', 1, 'earned');
      if (!success) {
        console.warn('Failed to send collaborative score update to other users');
      }
    }
    ...
    if (collaborativeMode && isConnected) {
      const success = updateCollaborativeScore('opponent', 1, 'our_error');
      if (!success) {
        console.warn('Failed to send collaborative score update to other users');
      }
    }
...
}, [handlePlayerAction, courtPlayers, isPlayerActive, triggerActionFeedback, showActionToast]);
...
const handleCollaborativeIgnoreKill = useCallback(async () => {
  ...
  // Store assist player and serve side
  const player = assistWaitingPlayer;
  const effectiveServeSide = assistWaitingServeSideRef.current || currentServeSide;
  const wasOpponentServing = effectiveServeSide === "opponent";
...
const renderCollaborativeControls = () => {
  ...
  return (
    <div style={{
      ...
      {/* Collaborative Mode Toggle - OWNERS ONLY, ALWAYS VISIBLE */}
      <div style={{
        backgroundColor: '#fff',
        borderRadius: '12px',
        padding: '12px',
        ...
        {collaborativeMode && (
          <div style={{
            ...
            {isCollaborativeReady() ? 
              'Ready for collaborative logging' : 
              `Not ready: ${!isConnected ? 'No connection' : 'Missing session data'}`
            }
          </div>
        )}
        
        {/* Show user status for debugging - only if no user */}
        {!user && (
          <div style={{
            marginTop: '8px',
            padding: '6px 10px',
...
const togglePlayerActivation = useCallback((playerIndex) => {
  ...
  const updateLocalScore = useCallback((team, pointType, wasOpponentServing) => {
    try {
      if (team === 'our') {
        onOurPoint && onOurPoint(true);
...
const debugScoringState = useCallback(() => {
  ...
  console.log('  Current Serve Side:', currentServeSide);
  console.log('  Ball State:', ballState);
  console.log('  Can Score?', !(collaborativeMode && !isConnected && !isCollaborativeReady()));
}, [collaborativeMode, isConnected, isCollaborativeReady, lastScoreUpdate, currentServeSide, ballState]);
...
const handlePointAwarded = useCallback((pointType) => {
  ...
  // ✅ ONLY CHECK COOLDOWN IN COLLABORATIVE MODE
  if (collaborativeMode && scoreCooldownActive) {
  ...
  if (team === 'our') {
    ...
    if (reason === 'earned') {
      setTeamStats && setTeamStats(prev => ({ ...prev, ourEarned: prev.ourEarned + 1 }));
    } else if (reason === 'opponent_error') {
      setTeamStats && setTeamStats(prev => ({ ...prev, oppError: prev.oppError + 1 }));
    }
  } else {
    onOpponentPoint && onOpponentPoint(true);
    if (reason === 'opponent_earned') {
      setTeamStats && setTeamStats(prev => ({ ...prev, oppEarned: prev.oppEarned + 1 }));
    } else if (reason === 'our_error') {
      setTeamStats && setTeamStats(prev => ({ ...prev, ourError: prev.ourError + 1 }));
    }
  }

  // 🔥 FIX: Update serve side FIRST, then ball state
  // This ensures the correct serve side is captured for collaborative sync
  setCurrentServeSide(team);
  setBallSide(team);
  setBallState("serve");
  
  // Now sync to collaborative mode with the CORRECT values
  if (collaborativeMode && isConnected) {
    // Sync serve side change
    updateCollaborativeServeSide(team);
    
    // Sync ball state with the NEW serve side
    updateCollaborativeBallState("serve", {
      ballSide: team,
      currentServeSide: team
    });
  ...
  addActionLogEntry(
    ...
    {
      type: 'manual_point',
      meta: { pointType, source: 'local', timestamp: now }
    }
  );
...
const completeAssistKillSequence = useCallback(async (assistPlayer, killPlayer, currentServeSideSnapshot = null) => {
  ...
  try {
    ...
    // ✅ REMOVED: Auto-assignment logic - just log the stats directly
    
    // Log stats for both players
    console.log('📊 Logging assist stat for:', assistPlayer.name);
    await logStat(assistPlayer, "Assist", ["sets", "assists"], 
      `${assistPlayer.name} (#${assistPlayer.number}) → Assist`);
    
    await new Promise(resolve => setTimeout(resolve, 200));
    
    console.log('📊 Logging kill stat for:', killPlayer.name);
    await logStat(killPlayer, "Kill", ["kills", "attacks", "points"],
    ...
    if (onOurPointRef.current) {
      onOurPointRef.current(true);
    }
    
    if (setTeamStatsRef.current) {
      setTeamStatsRef.current(prev => {
        const newStats = { ...prev, ourEarned: prev.ourEarned + 1 };
        return newStats;
      });
    }
    ...
    if (collaborativeMode && isConnected) {
      ...
      // Notify others that assist-kill is complete
      if (socket) {
...
}, [courtPlayers, isPlayerActive, completeAssistKillSequence]);
...
const renderConnectionStatus = () => {
  ...
  const playerCardStyle = (isActive) => ({
    ...
    display: 'grid',
    gridTemplateColumns: 'minmax(150px, 150px) 1fr auto',  // Fixed first column!
            ...
            <button 
              ...
              onClick={() => {
                if (ballSide !== 'our') {
                  ...
                  setCurrentServeSideWithSync('our');
                  addActionLogEntry("Service side manually changed to Our team (no rotation)", {
                    type: 'manual_serve_change',
                    meta: { newSide: 'our' }
                  });
                }
              }}
            ...
            <button 
              ...
              onClick={() => {
                if (ballSide !== 'opponent') {
                  ...
                  setCurrentServeSideWithSync('opponent');
                  addActionLogEntry("Service side manually changed to Opponent (no rotation)", {
                    type: 'manual_serve_change',
                    meta: { newSide: 'opponent' }
                  });
                }
              }}
            >
          ...
          <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button 
              ...
              onClick={() => {
                if (ballState !== 'serve') {
                  setBallStateWithSync('serve');
                  setCurrentServeSideWithSync(ballSide);
                  addActionLogEntry(
                    `Game state manually changed to Serve (${ballSide === 'our' ? 'Our' : 'Opponent'} serve)`,
                    {
                      type: 'manual_state_change',
                      meta: { newState: 'serve', serveSide: ballSide }
                    }
                  );
                }
              }}
            ...
            <button 
              ...
              onClick={() => {
                if (ballState !== 'inplay') {
                  setBallStateWithSync('inplay');
                  addActionLogEntry(
                    "Game state manually changed to In Play (Rally)",
                    {
                      type: 'manual_state_change',
                      meta: { newState: 'inplay' }
                    }
                  );
                }
              }}
...
{pendingPassGrade && (
  ...
  >
    ...
    >
      {(() => {
        const player =
          pendingPassGrade && courtPlayers[pendingPassGrade.playerIndex];
        const label =
          pendingPassGrade?.actionType === 'REC'
            ? 'Serve Receive'
            : 'Dig';
        return (
          <>
      ...
      >
        {[1, 2, 3].map((grade) => (
          <button
            key={grade}
            onClick={() => handlePassGradeSelect(grade)}
            style={{
              width: 60,
              height: 60,
              borderRadius: 999,
              ...
              <DraggablePlayerCard player={player} index={index}>
                <div style={{
                  ...
                  opacity: (collaborativeMode && trackingStatus?.isAssigned && !trackingStatus?.isMe) ? 0.6 : 1
				   ...
				   {/* Show collaborative player tracking to all users in collaborative mode */}
{collaborativeMode && player._id && (
  ...
  />
		  ...
		  {/* Show tracking overlay to all users in collaborative mode */}
...
{/* Collaborative Block Waiting Overlay (for non-initiators) */}
{collaborativeBlockState.active && !blockMode && (
  <div
    style={{
      position: 'fixed',
      top: 0,
      ...
      bottom: 0,
      backgroundColor:
        collaborativeBlockState.type === 'assist'
          ? 'rgba(40, 167, 69, 0.95)'
          : 'rgba(220, 53, 69, 0.95)',
      zIndex: 9999,
      pointerEvents: 'auto',
      // --- Changes for Centering and Scrollability ---
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      padding: '20px', // Add safe area/gutter
      overflowY: 'auto' // Allow the whole backdrop to scroll if needed
      // --- End Changes ---
    }}
  >
    <div
      style={{
        ...
        // --- Added Margin to balance centering/scrolling ---
        margin: 'auto 0'
        // --- End Addition ---
      }}
    >
      <div
        style={{
          ...
          fontWeight: '800',
          color:
            collaborativeBlockState.type === 'assist' ? '#28a745' : '#dc3545',
          marginBottom: '16px'
        }}
      >
        {collaborativeBlockState.timer}s
      </div>
      ...
      >
        Block {collaborativeBlockState.type === 'assist' ? 'Assist' : 'Error'}{' '}
        Selection
      </div>
      ...
      >
        **PLEASE WAIT** :{' '}
        {collaborativeBlockState.initiator?.username || 'Another user'} is
        completing the block action for all front row players
      </div>

      {collaborativeBlockState.players.length > 0 && (
        <div
          style={{
            fontSize: '14px',
            color: '#333',
        ...
        >
          <strong>Selected ({collaborativeBlockState.players.length}):</strong>
          <br />
          {collaborativeBlockState.players
...
{/* Block Assist Error Buttons */}
{blockMode && !collaborativeBlockState.active && (
  <div style={{ // OUTER MODAL CONTAINER (Full screen fixed overlay)
    position: 'fixed',
    top: 0,
    left: 0,
    right: 0,
    ...
    // CRITICAL FIX 1: Make the full-screen overlay the scroll handler
    overflowY: 'auto', 
    WebkitOverflowScrolling: 'touch', // Enables smooth momentum scrolling on iOS
    
    // CRITICAL FIX 2: Removed all flex centering. Use block flow instead.
    display: 'block', 
  }}>
    <div style={{ // INNER MODAL WRAPPER (The visible content box)
      backgroundColor: '#ffffff',
      borderRadius: '12px',
      padding: '20px', // Standard padding for content
      maxWidth: '500px',
      width: '95%',
      
      // CRITICAL FIX 3: Center horizontally and provide vertical margin for scrolling
      margin: '20px auto', // Centers the modal and provides 20px of space top/bottom
      
      // Removed all height constraints to let content flow naturally
      maxHeight: 'none',   
      overflowY: 'visible', 
      ...
      {/* Header and Info Section (Pulled out to the edges with negative margin) */}
      <div style={{ 
        paddingBottom: '20px', 
        backgroundColor: '#50729F', 
        color: '#ffffff', 
        // Use negative margin to pull the colored block to the wrapper edges
        margin: '-20px -20px 20px -20px', 
        borderRadius: '12px 12px 0 0',
        padding: '20px' // Add padding back for content inside the colored box
      }}>
         <h3
          style={{
            marginTop: 0,
        ...
        >
          Block {blockMode === 'assist' ? 'Assist' : 'Error'} Selection
        </h3>
        ...
        >
          {blockMode === 'assist'
            ? '✓ Click all front row players who participated in the block assist'
            : '⚠️ Click all front row players who made blocking errors'}
        </div>

        {/* Secondary info box (Points Info) */}
        <div 
          style={{
            marginTop: '16px',
            padding: '8px',
        ...
        >
          ...
          >
            {blockMode === 'assist'
              ? 'Each player gets 0.5 points • Team scores 1 point'
              : 'No points awarded • Opponent scores'}
          </div>
        </div>
      </div>
      ...
      >
        ...
        {/* Player List Container - must be visible */}
        <div
          style={{
            display: 'flex',
            flexDirection: 'column',
        ...
        >
          {courtPlayers.map((player, index) => {
            ...
            // Only show front row players
            if (
              !isActive ||
              !player ||
              !player.name ||
              player.name === '?' ||
              !['2', '3', '4'].includes(currentPosition) ||
              player.isLibero
            ) {
              return null;
            }
            ...
            return (
              <div
                key={index}
                onClick={() => canSelect && handleBlockPlayerToggle(index)}
                style={{
                  padding: '12px',
                  borderRadius: '8px',
                  border: `2px solid ${
      ...
      >
        <button
          ...
          style={{
            ...
            fontSize: '14px',
            cursor:
              selectedBlockPlayers.size > 0 ? 'pointer' : 'not-allowed',
            transition: 'all 0.15s ease'
          }}
        >
          Submit Block {blockMode === 'assist' ? 'Assist' : 'Error'} (
          {selectedBlockPlayers.size})
        </button>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ 
          ...
          {(() => {
            const { firstLine, secondLine } = splitNameForDisplay(player.name, 12);
            return (
              <div>
                <div>{firstLine}</div>
        ...
        {player.isLibero && (
          <div style={{ fontSize: '9px', color: '#666' }}>(L)</div>
        )}
      </div>
    </div>
  ) : (
                  ...
                  {isActive && actionGroups.length > 0 && (
                    <div style={actionButtonsStyle}>
                      {actionGroups.map((group, groupIndex) => (
						...
						<div style={{
						display: 'flex',
						alignItems: 'center',    // horizontal centering of the rotated element
						justifyContent: 'center',// vertical centering of the rotated element
						height: '100%',          // make sure the container fills available height
						}}>
						<div style={{
							fontSize: '8px',
							color: '#666',
							...
							transform: 'rotate(270deg)',
							textAlign: 'center',
							transformOrigin: 'center center', // ensure rotation stays centered
						}}>
                            {group.category}
                          </div>
						  </div>
                          ...
                          {group.actions.map(action => (
                            <button
                              key={action.key}
                              onClick={() => handlePlayerActionWithFeedback(index, action.key)}
                              onMouseDown={(e) => {
                              ...
                              style={getActionButtonStyleWithFeedback(action.variant, `player-${index}-${action.key}`)}
                            >
                              {action.label}
                            </button>
                          ))}
...
{blockAssistMode && (
  <div style={{ // OUTER MODAL CONTAINER (Full screen overlay)
    position: 'fixed', 
    top: 10, 
    left: 0, 
    right: 0, 
    ...
    display: 'flex', 
    justifyContent: 'center', 
    alignItems: 'flex-start', // CRITICAL FIX 1: Align content to the top       // CRITICAL FIX 2: Allow the whole fixed layer to scroll
    paddingTop: '20px',      // Provide padding at the top for appearance
    paddingBottom: '20px',   // Ensure padding at the bottom for scrollability
    zIndex: 9998 
  }}>
    <div style={{ // INNER MODAL CONTAINER (The white content box)
      backgroundColor: '#50729F',
      borderRadius: '12px',
      padding: '20px',
      maxWidth: '500px',
      width: '95%',
      // CRITICAL FIX 3: REMOVE ANY HEIGHT OR SCROLLING CONSTRAINTS
      // This allows the content to dictate the full height, enabling the outer DIV to scroll.
      maxHeight: '25',     // Ensures no maximum height is set (or delete the line entirely)
      overflowY: 'auto',  // Ensures the content is not clipped (or delete the line entirely)

      WebkitOverflowScrolling: 'touch', // Keep for mobile performance
      touchAction: 'pan-y',             // Keep for mobile performance
      overscrollBehavior: 'contain',
      boxShadow: '0 4px 12px rgba(0,0,0,0.15)'
    }}>
      <h3 style={{ 
          ...
          <div style={bottomBarSectionStyle}>
            ...
            <div style={bottomBarButtonRowStyle}>
              <button
                ...
                disabled={collaborativeMode && !isConnected || !scoringEnabled}
                style={getBottomBarButtonStyleWithFeedback('#28a745', '#ffffff', 'point-ourEarned')}
              >
                We Earned
              </button>
              
               <button
                onClick={() => handlePointAwardedWithFeedback('opponentError')}
                disabled={collaborativeMode && !isConnected || !scoringEnabled}
                style={getBottomBarButtonStyleWithFeedback('#ffc107', '#212529', 'point-opponentError')}
              >
                Opp Error
              </button>
            </div>
          ...
          <div style={bottomBarSectionStyle}>
            ...
            <div style={bottomBarButtonRowStyle}>
              <button
                ...
                disabled={collaborativeMode && !isConnected || !scoringEnabled}
                style={getBottomBarButtonStyleWithFeedback('#dc3545', '#ffffff', 'point-opponentEarned')}
              >
                They Earned
              </button>
              
              <button
                onClick={() => handlePointAwardedWithFeedback('ourError')}
                disabled={collaborativeMode && !isConnected || !scoringEnabled}
                style={getBottomBarButtonStyleWithFeedback('#fd7e14', '#ffffff', 'point-ourError')}
              >
                Our Error
              </button>
            </div>
