...
const DraggableBenchCard = ({ 
  ...
  // Always call useDrag hook, but conditionally enable/disable drag functionality
  const [{ isDragging }, drag] = useDrag(
    () => ({
      type: "PLAYER",
      item: canSub ? (() => {
        // Hide serve zone overlay only if it's our serve
        if (currentServeSide === "our") {
          setShowServeZoneOverlay(false);
        }
        return player;
      }) : null, // Return null when dragging is disabled
      collect: (monitor) => ({
        isDragging: monitor.isDragging(),
      }),
      end: (item, monitor) => {
        // Set overlay back to true when drag ends
        if (canSub && currentServeSide === "our") {
          setShowServeZoneOverlay(true);
        }
      },
      canDrag: canSub, // This properly disables dragging when canSub is false
    }),
    [player, currentServeSide, setShowServeZoneOverlay, canSub]
  );

  return (
    <div
      ref={canSub ? drag : null} // Only attach drag ref when dragging is enabled
      style={{
...
function VolleyballCourt({
  ...
  const [mobileActiveTab, setMobileActiveTab] = useState('court'); 
...
const getContextualHints = () => {
  if (!voiceEnabled) {
    if (!isSubscriber && user?.role !== 'admin') {
  ...
  } else if (!isSubscriber && user?.role !== 'admin') {
...
const { isListening, lastCommand } = useSimpleVoiceCommands(
  ...
  courtPlayers,
  voiceEnabled && (isSubscriber || user?.role === 'admin'),
  {
    ballState,
    currentServeSide, 
...
/**
 * LEFT SIDEBAR STYLE - Scoreboard and controls (Desktop only)
 * Contains the scoreboard, team names, and key controls
 */
...
/**
 * CENTER COURT AREA STYLE - Main court display area
 * Optimized height for mobile, flexible for desktop
 */
const centerCourtStyle = {
  display: 'flex',
  flexDirection: 'column',
  position: 'relative',
  flex: isMobile ? '0 0 auto' : '1',
  
  // Mobile: 50-70% of viewport height (accounting for header, tabs, toolbar)
  ...(isMobile && {
    width: '100%',
    minHeight: '50vh',
    maxHeight: '65vh',
  ...
  // Desktop: flexible height to fill available space
  ...(!isMobile && {
    flex: '1 1 auto',
...
/**
 * RIGHT SIDEBAR STYLE - Logs and Bench panels (Desktop only)
 * Displays history log and bench players side by side on desktop
 */
...
/**
 * MOBILE TAB NAVIGATION - Tabs to switch between Court/Bench/History
 * Only visible on mobile devices
 */
const mobileTabContainerStyle = {
  ...
  position: 'sticky',
  top: isMobile && showHeader ? '60px' : '0',
  zIndex: '50',
  justifyContent: 'center',
  flexWrap: 'nowrap',
  overflowX: 'auto',
...
/**
 * MOBILE TAB BUTTON STYLE - Individual tab buttons
 * Creates visual feedback for active/inactive states
 */
const getMobileTabButtonStyle = (isActive) => ({
  flex: '1 0 auto',
  minWidth: '70px',
  padding: '10px 16px',
  border: 'none',
  borderRadius: '6px',
  backgroundColor: isActive ? '#007AFF' : '#e8e8e8',
  ...
  '&:hover': {
    backgroundColor: isActive ? '#0051D5' : '#d0d0d0',
  },
...
/**
 * MOBILE UTILITY TOOLBAR - Fixed toolbar at bottom with context-aware buttons
 * Shows rotate, undo/clear, switch serve based on game state
 */
const mobileUtilityToolbarStyle = {
  display: isMobile ? 'flex' : 'none',
  position: 'fixed',
  bottom: '0',
  left: '0',
...
/**
 * MOBILE UTILITY BUTTON STYLE - Compact buttons for toolbar
 */
const mobileUtilityButtonStyle = {
  padding: '10px 14px',
  fontSize: '12px',
  fontWeight: '600',
...
/**
 * DESKTOP UTILITY PANEL STYLE - Groups utility buttons in sidebar
 */
const desktopUtilityPanelStyle = {
  display: isMobile ? 'none' : 'flex',
  flexDirection: 'column',
  gap: '8px',
  padding: '12px',
...
/**
 * MOBILE TAB CONTENT CONTAINER - Wrapper for tab content
 * Handles padding to avoid overlap with toolbar
 */
const mobileTabContentStyle = {
  flex: '1',
  overflowY: 'auto',
  overflowX: 'hidden',
  paddingBottom: '90px', // Space for fixed toolbar
  paddingTop: '8px',
  paddingLeft: '12px',
  paddingRight: '12px',
  display: 'flex',
...
/**
 * ADJUSTED BENCH PANEL STYLE for responsive layout
 * Remove fixed sizing on desktop, adjust for mobile
 */
const responsiveBenchPanelStyle = {
  flexGrow: 1,
...
/**
 * ADJUSTED LOGS PANEL STYLE for responsive layout
 * Make desktop version scrollable, remove fixed sizing
 */
const responsiveLogsPanelStyle = {
  display: 'flex',
  flexDirection: 'column',
  gap: '10px',
  
  // Desktop: scrollable right column
  ...(!isMobile && {
    flex: '1 1 auto',
    minHeight: '0',
...
/**
 * ADJUSTED LOG CARD STYLE for responsive layout
 */
const responsiveLogCardStyle = {
  backgroundColor: '#f9f9f9',
  borderRadius: '6px',
  border: '1px solid #e5e5e5',
  boxShadow: 'inset 0 1px 3px rgba(0,0,0,0.05)',
...
const benchGridStyle = {
  ...
  const benchCardActiveStyle = {
    transform: "scale(0.95)", // Subtle tap/drag effect
  };
...
const slotStyle = (index, player, flash) => {
  const size = isMobile && deviceInfo.isLandscape ? 80 : (isMobile ? 90 : 100);
...
const logsPanelStyle = {
  width: isMobile ? "180px" : "220px", // Fixed width
  ...
  maxHeight: isMobile ? "160px" : "210px",
  };
...
const ActionZone = ({ label, zoneAction, ballSide }) => {
...
return (
    <button onClick={() => handleActionDrop(zoneAction)} style={dynamicStyle}>
      {label}
    </button>
  );
};
  ...
  // Render action zones. When "In Play" is tapped, block circles appear.
function renderActionZones() {
  const zoneButtons = [];

  const commonActions = (actions) =>
    actions.map(({ label, zoneAction }) => (
      <ActionZone
        key={zoneAction}
        label={label}
  ...
  if (blockInfo) {
    ...
    if (currentServeSide === "our") { // CHANGED: was serveSide === "our"
      zoneButtons.push(
        ...commonActions([
    ...
    } else if (currentServeSide === "opponent") { // CHANGED: was serveSide === "opponent"
      zoneButtons.push(
        ...commonActions([
    ...
    zoneButtons.push(
      ...commonActions([
        { label: "Opponent Kill", zoneAction: "Kill" },
        { label: "Opponent Error", zoneAction: "Error" },
      ])
    );
    ...
    if (ourTouches === 1) {
      zoneButtons.push(
        ...commonActions([
          { label: "Reception Error", zoneAction: "Error" },
          { label: "Kill", zoneAction: "Kill" },
          { label: "In Play", zoneAction: "InPlay" },
        ])
      );
      ...
      zoneButtons.push(
        ...commonActions([
          { label: "Set Error", zoneAction: "Error" },
          { label: "Kill", zoneAction: "Kill" },
          { label: "In Play", zoneAction: "InPlay" },
        ])
      );
      ...
      zoneButtons.push(
        ...commonActions([
          { label: "Attack Error", zoneAction: "Error" },
          { label: "Kill", zoneAction: "Kill" },
          { label: "In Play", zoneAction: "InPlay" },
        ])
      );
...
const handlePlayerDrop = async (benchPlayer, slotIndex) => {
  ...
  if (isLiberoSub) {
    ...
    if (isTargetLibero && benchPlayer.isLibero) {
      ...
      // 🎯 SIMPLIFIED GAMES PLAYED LOGIC FOR LIBERO SWAP
      if (isBeforeSetStart) {
        // Credit incoming libero
        // await maybeCreditGamesPlayed(benchPlayer._id, true);
		// creditedPlayersThisSetRef.current.add(benchPlayer._id);
        
        // Remove credit from outgoing libero (they're going back to bench)
        if (outgoingLibero._id) {
          await removeGamesPlayedCredit(outgoingLibero._id);
		  creditedPlayersThisSetRef.current.delete(outgoingLibero._id);
        }
      }
    ...
    if (!currentLibero && !allowedLiberoSubTarget) {
      ...
      // 🎯 SIMPLIFIED GAMES PLAYED LOGIC FOR FIRST LIBERO SUB
      if (isBeforeSetStart) {
        // Credit incoming libero
        await maybeCreditGamesPlayed(benchPlayer._id, false);
		creditedPlayersThisSetRef.current.add(benchPlayer._id);
        
        // Do NOT remove credit from replaced player - they go to bench but keep their games played
      }
...
if (isBeforeSetStart) {
  ...
  creditedPlayersThisSetRef.current.add(benchPlayer._id);
  console.log(`${benchPlayer._id} added to Gameflow Cursor`);
  
  // Don't remove credit from replaced player - they keep their games played
  }
...
return;
  ...
  // =============================
    ...
    // 🎯 SIMPLIFIED GAMES PLAYED LOGIC FOR REGULAR SUB
    if (isBeforeSetStart) {
...
const CourtSlot = React.forwardRef(({ player, index, flash }, ref) => {
 ...
 const { line1, line2 } = splitPlayerName(player.name || "", 10);
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        lineHeight: line2 ? "1.1" : "1.2", // Tighter line height if two lines
        fontSize: line2 ? "0.8rem" : "0.9rem", // Smaller font if two lines
        textAlign: "center"
      }}>
        <div>{line1}</div>
        {line2 && <div>{line2}</div>}
      ...
      <div style={slotNumberStyle}>#{player.number}</div>
      <div style={slotPosStyle}>Pos {positionLabels[index]}</div>
      {positionLabels[index] === "1" && !player.isLibero && (
        <div style={serverBadgeStyle}>Server</div>
      )}
      {player.isLibero && <div style={liberoBadgeStyle}>Libero</div>}
    </div>
  );
});
...
const resetRally = (wonBy) => {
  setBallState("serve");
  setCurrentServeSide(wonBy); // "our" or "opponent"
  onOpponentPoint();
  setTouches([]);
  setFreeBallModifier(null);
  setBlockCirclesVisible(false);
...
const getNetBanner = (eventName) => {
  ...
  const label = (eventName || (isSubscriber ? "LOGGERHEAD PRO" : "LOGGERHEAD FREE")).toUpperCase().trim();
          ...
          {benchPlayers &&
            benchPlayers
              ...
              .map((player, i) => (
				...
				setShowServeZoneOverlay={setShowServeZoneOverlay}  // Pass this down
/>
  ...
  return (
    <div ref={containerRef} style={courtAreaStyle}>
      ...
      <VolleyballIcon
        ...
        serverSlotPosition={serverSlotPosition}
        netPosition={{ top: "35%", left: "35%" }}
        inPlayPosition={inPlayPosition}
        ballState={ballState}
    ...
    <div style={{ 
      ...
      {/* DESKTOP THREE-COLUMN LAYOUT */}
        ...
        <div style={mainContainerStyle}>
          {/* LEFT SIDEBAR: Scoreboard and Controls */}
          <div style={leftSidebarStyle}>
            {renderScoreboardAboveBench()}
          
          </div>
          {/* CENTER: Court Area */}
          <div style={centerCourtStyle}>
            {renderCourtArea()}
          </div>
          
          {/* RIGHT SIDEBAR: Logs and Bench */}
          <div style={rightSidebarStyle}>
            {/* Bench Panel */}
            <div style={{
              ...responsiveBenchPanelStyle,
              flex: '0 0 auto',
              minHeight: '180px',
            }}>
              <h3 style={{
      ...
      {isMobile && (
        <>
          ...
          <div style={mobileTabContainerStyle}>
            <button
              style={getMobileTabButtonStyle(mobileActiveTab === 'court')}
              onClick={() => setMobileActiveTab('court')}
            >
              Court
            </button>
            <button
              style={getMobileTabButtonStyle(mobileActiveTab === 'bench')}
              onClick={() => setMobileActiveTab('bench')}
            >
              Bench
            </button>
            <button
              style={getMobileTabButtonStyle(mobileActiveTab === 'history')}
          ...
          <div style={{
            ...
            overflowX: 'hidden',
            paddingBottom: '90px',
            display: mobileActiveTab === 'court' ? 'flex' : 'none',
            flexDirection: 'column'
          }}>
            {renderCourtArea()}
          </div>
          ...
          <div style={{
            ...
            paddingBottom: '90px',
            padding: '12px',
            display: mobileActiveTab === 'bench' ? 'flex' : 'none',
            flexDirection: 'column'
          }}>
            {renderScoreboardAboveBench}
          </div>
          ...
          <div style={{
            ...
            paddingBottom: '90px',
            padding: '12px',
            display: mobileActiveTab === 'history' ? 'flex' : 'none',
            flexDirection: 'column'
          }}>
            {renderLogsPanel()}
          </div>
          ...
          {ballState === 'serve' && (
            <div style={mobileUtilityToolbarStyle}>
              <button 
      ...
      {ballState === "serve" && (
        <div
          style={{
            position: "absolute",
            top: isMobile && deviceInfo.isLandscape ? "120px" : "120px",
            // left: isMobile && deviceInfo.isLandscape ? "25%" : "48%",
            right: isMobile && deviceInfo.isLandscape ? "250px" : "250px",
            ...
            maxWidth: isMobile && deviceInfo.isLandscape ? "100px" : "60px"
          }}
        >
...
<div
  style={{
    position: "fixed",
    left: "calc(50% - 240px)", // ~center of middle space
    top: isMobile && deviceInfo.isLandscape ? "40%" : (showHeader ? "102px" : "19px"),
    width: isMobile && deviceInfo.isLandscape ? "480px" : "500px",
    maxWidth: "90vw",
    height: isMobile && deviceInfo.isLandscape ? "80dvh" : (mobileHeader ? "90dvh" : "450px"),
    transform: isMobile && deviceInfo.isLandscape ? "translateY(-50%)" : "none",
    ...
    overflow: isMobile && deviceInfo.isLandscape ? "auto" : "visible"
      ...
      >
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns: isMobile && deviceInfo.isLandscape ? "repeat(5, 50px)" : "repeat(5, 70px)",
            gridGap: isMobile && deviceInfo.isLandscape ? "6px" : "10px",
            justifyContent: "center"
          }}
        >
          {(serveZoneMode === 6
            ? ["1", "1-6", "6", "6-5", "5", "2", "2-3", "3", "3-4", "4"]
            : ["1", "1-6", "6", "6-5", "5", "2", "2-3", "3", "3-4", "4"]
          ).map((zone) => {
            const isDashed = zone.includes("-");
            return (
              <button
                key={zone}
                onClick={() => {
                  setSelectedServeZone(zone);
                  setShowServeZoneOverlay(false);
                }}
                style={{
                  width: isMobile && deviceInfo.isLandscape ? "50px" : "70px",
                  height: isMobile && deviceInfo.isLandscape ? "35px" : (mobileHeader? "40px" : "80px"),
                  fontSize: isMobile && deviceInfo.isLandscape ? "0.6rem" : (isDashed ? ".7rem" : "2.0rem"),
                  fontWeight: "bold",
                  borderRadius: "12px",
                  backgroundColor: isDashed ? "#34C759" : "#007AFF",
                  color: "#fff",
                  border: "none",
                  cursor: "pointer",
                  display: "flex",
...
/>
    ...
    {errorContext && (
      ...
      >
        ...
        {errorContext.touchCount === 1 && (
          <>
            <button
              style={iosButtonStyle}
              onClick={() => {
                setPendingErrorType("Receiving");
                setAwaitingRefBlownDecision(true);
              }}
            >
              Receiving Error
        ...
        {errorContext.touchCount === 2 && (
          <>
            <button
              style={iosButtonStyle}
              onClick={() => {
                setPendingErrorType("Setting");
                setAwaitingRefBlownDecision(true);
              }}
            >
              Setting Error
