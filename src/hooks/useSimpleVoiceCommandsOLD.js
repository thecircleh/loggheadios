...
const useSimpleVoiceCommands = (handleCommand, courtPlayers, voiceEnabled, gameState = {}) => {
  ...
  // Enhanced smart parsing for concatenated player numbers
  const parsePlayerNumbers = (numberStr, courtPlayers) => {
    ...
    // Only proceed with complex parsing for multi-character strings
    ...
    // Recursive parsing for 2-3 character strings
    const parseRecursive = (str, position = 0, result = []) => {
      ...
      // Try 2-digit numbers first, then 1-digit
  ...
  // Enhanced force integer conversion for voice commands
  const forceToInteger = (text) => {
    ...
    // Comprehensive word-to-digit conversion
    const quickMap = {
      'zero': '0',
      'one': '1', 'won': '1',
      'two': '2', 'to': '2', 'too': '2', 
    ...
    // Handle sequences with mixed words and numbers like "six fifteen nine kill"
  ...
  const parseSimpleCommand = (transcript) => {
    ...
    // ALWAYS try to force integer conversion first
    ...
    // 🔧 FIX: Use converted text for zone parsing to handle word numbers
    ...
    // *** CONTEXT-AWARE ZONE PARSING - FIXED TO USE CONVERTED TEXT ***
    // Only parse as serve zone if we're actually in serve zone selection mode
    ...
    if (zoneMatch && gameState.showServeZoneOverlay && gameState.ballState === "serve") {
      ...
      // Fall through to try other parsing options
    }
    ...
    // 🔧 ALSO FIX: Enhanced service error detection for iOS Safari
    // Service error types - handle both word and digit forms
    ...
    if (serviceErrorMatch || originalText.match(/^(out|in\s*net|net|foot\s*fault|in\s+net)$/)) {
      // Only parse as service error if we're in error modal or serve zone overlay
      ...
      // Otherwise treat as generic error result
      return {
        type: "result_only",
        result: "error"
      };
    ...
    // 🔧 FIXED: Handle alternative phrasings like "service error out" or "serve error net"
    if (originalText.match(/^(service\s+error|serve\s+error)\s+(out|net|foot\s*fault)$/)) {
      const errorPart = originalText.split(/\s+/).slice(-1)[0]; // Get last word
    ...
    // 🔧 FIXED: Handle "foot fault" as two words
    ...
    // Enhanced number sequences with results - CONTEXT CHECK
    // Now primarily expects word-based input like "six fifteen nine kill"
    ...
    if (sequenceWithResultMatch) {
      ...
      console.log("🔍 After word-to-number conversion:", numberPart);
      ...
      if (allowSequence) {
        ...
        // Parse space-separated numbers (now should be clean digits)
        numbers = numberPart.split(/\s+/)
          .map(s => parseInt(s))
          .filter(n => !isNaN(n));
        
        console.log("🔍 Final parsed numbers:", numbers);
        ...
        if (validNumbers.length >= 2) {
          console.log("✅ Valid sequence found:", { numbers: validNumbers, result });
          return {
            type: "sequence",
    ...
    // Enhanced number sequences without results - CONTEXT CHECK  
    // Now primarily expects word-based input like "six fifteen nine"
    ...
    if (sequenceMatch) {
      ...
      console.log("🔍 After word-to-number conversion:", numberString);
      ...
      if (allowSequence) {
        ...
        if (numberString.includes(' ')) {
          // Space-separated numbers (should be clean after word conversion)
          numbers = numberString.split(/\s+/)
            .map(s => parseInt(s))
            .filter(n => !isNaN(n));
        } else {
          // Single concatenated string: try smart parsing as fallback
        ...
        if (validNumbers.length >= 2) {
          console.log("✅ Multi-player sequence in correct context:", validNumbers);
          return {
    ...
    // Block actions - enhanced with player number support
    if (gameState.ballState === "inplay" && gameState.blockCirclesVisible) {
      // Original simple block commands (keep existing functionality)
      ...
      // NEW: Enhanced block sequences with player numbers
      // Pattern 1: "block [result] [numbers]" - e.g., "block kill 1 2" or "block kill 5"
      ...
      if (blockFirstMatch) {
        ...
        if (numberPart.includes(' ')) {
          ...
          // Concatenated - use smart parsing (only for multi-digit strings)
      ...
      // Pattern 3: Single player block - "[number] block [result]" or "block [result] [number]"
    ...
    // Simple action words - context-aware
    const actions = {
  ...
  useEffect(() => {
    if (!voiceEnabled) {
      ...
      consecutiveRestarts.current = 0; // Reset restart counter
      return;
    }
    ...
    console.log("🎙️ Setting up new voice recognition");
    const recognition = new SpeechRecognition();
    recognitionRef.current = recognition;

    // Configuration optimized for Samsung tablets
    ...
    recognition.onstart = () => {
      ...
      isRestartingRef.current = false;
      consecutiveRestarts.current = 0; // Reset on successful start
    ...
    recognition.onend = () => {
      ...
      if (!voiceEnabled || isRestartingRef.current) {
        console.log("🎙️ Not restarting - voice disabled or manual stop");
        isRestartingRef.current = false;
        return;
      }
      ...
      // Prevent rapid restart loops with device-specific delays
      const now = Date.now();
      const timeSinceLastRestart = now - lastRestartTime.current;
      const minRestartDelay = isAndroidTablet.current ? 3000 : 2000; // Longer delay for tablets
      
      if (timeSinceLastRestart > minRestartDelay) {
        console.log("🎙️ Auto-restarting voice recognition after silence");
        isRestartingRef.current = true;
        lastRestartTime.current = now;
        consecutiveRestarts.current += 1;
        
        // Device-specific restart delay
        ...
        setTimeout(() => {
          if (voiceEnabled && !recognitionRef.current) {
            try {
              ...
              console.log("🎙️ Voice recognition restarted successfully");
            } catch (error) {
    ...
    recognition.onresult = (event) => {
      ...
      // For Samsung tablets in single-shot mode, restart after processing
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error === 'no-speech') {
        ...
        // Don't treat no-speech as a real error, it will auto-restart via onend
        return;
      }
      ...
      // For other errors, set failed status and allow manual restart
      console.log("🎙️ Setting status to failed due to error:", event.error);
      setConnectionStatus("failed");
      consecutiveRestarts.current += 1;
      
      // Only auto-restart for network errors after a longer delay on tablets
  ...
  }, [voiceEnabled]); // Only depend on voiceEnabled

  const manualRestart = () => {
    console.log("🎙️ Manual restart requested");
    
    // Prevent rapid manual restarts
    const now = Date.now();
    const timeSinceLastRestart = now - lastRestartTime.current;
    ...
    // Restart after a device-appropriate delay
